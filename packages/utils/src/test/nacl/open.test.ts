...
import { naclBoxPairFromSecret, naclOpen, naclSeal } from '../../nacl'

describe('naclOpen', (): void => {
  test('opens a sealed message by the sender', (): void => {
  ...
  test('does double ratchet', () => {
    const sender = ed25519PairFromString('sender')
    const receiver = ed25519PairFromString('receiver')

    // Make id key for identifying each other
    const senderIdKey = keyFromPath(
      sender,
      keyExtractPath('//1//1//1//1').path,
      'ed25519'
    )
    const receiverIdKey = keyFromPath(
      receiver,
    ...
    const senderIdKey2 = keyFromPath(
      sender,
      keyExtractPath('//1//1//1//2').path,
      'ed25519'
    )
    const receiverIdKey2 = keyFromPath(
      receiver,
      keyExtractPath('//2//2//2//3').path,
      'ed25519'
    )
    ...
    let ctx = naclSeal(
      message2,
      senderIdBoxKey2.secretKey,
      receiverIdBoxKey2.publicKey
    )
    ...
    opened = naclOpen(
      sealed2,
      nonce2,
      senderIdBoxKey2.publicKey,
      receiverIdBoxKey2.secretKey
    )
    // console.log(`The sealed message is ${opened?.toString() || 'null'}`);

    // This time derive key for each message to send and receive
    // Receiver sends encrypting public key to receive message to decrypt with his private key
    const receiverIdBoxKey2One = naclBoxPairFromSecret(
      receiverIdBoxKey2.secretKey
    )
    ...
    ctx = naclSeal(
      message3,
      senderIdBoxKey2One.secretKey,
      receiverIdBoxKey2One.publicKey
    )
    ...
    opened = naclOpen(
      sealed2,
      nonce2,
      senderIdBoxKey2One.publicKey,
      receiverIdBoxKey2One.secretKey
    )
